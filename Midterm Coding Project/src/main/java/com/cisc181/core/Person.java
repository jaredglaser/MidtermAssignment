...
public abstract class Person implements java.io.Serializable {
	...
	private void validateAge(Date DOB) throws PersonException{
		long Birth = DOB.getTime();

		Date d = new Date(); //current time
		long oneHundredBefore = (long) (d.getTime() - 3.154e+11);
		if(Birth < oneHundredBefore){
	...
	public void setPhone(String newPhone_number) throws PersonException {
		...
		validatePhoneNumber(newPhone_number);
		phone_number = newPhone_number;


	}

	private void validatePhoneNumber(String phoneNum) throws PersonException{
		Pattern pattern = Pattern.compile("\\(\\d{3}\\)-\\d{3}-\\d{4}"); // REGEX for (XXX)-XXX-XXXX
